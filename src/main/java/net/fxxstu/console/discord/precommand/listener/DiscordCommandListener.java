package net.fxxstu.console.discord.precommand.listener;
...
public class DiscordCommandListener extends ListenerAdapter {
    ...
    public void onGuildMessageReceived(@NotNull GuildMessageReceivedEvent event) {
        ...
        if (!this.commandLoader.getCommands().containsKey((label = label.replaceFirst(prefix, "")))) {
            event.getChannel().sendMessage(":x: **|** " + event.getAuthor().getAsMention() + ", unknown command. **Contribute pull requests**: https://github.com/Fxxst-creator/bukkit-discord-console.").queue();
            return;
        }
